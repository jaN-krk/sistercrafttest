...
`APP_ORIGIN=https://sistercraftandco.com` kullanılır. Sitedeki iletişim adresi varsayılan olarak `info@sistercraftandco.com` olur; yönetimde kaydedilen adres veya `STORE_CONTACT_EMAIL` ile değiştirilebilir. `STORE_NOTIFICATION_EMAIL=ebr.krgl@gmail.com` mağaza içi bildirimlerin alıcısıdır. İletişim adresi, bildirim alıcısı ve `EMAIL_FROM` göndericisi ayrı ayarlardır; sitede adres yayımlamak posta kutusu oluşturmaz. `.env.example` gerekli değişkenlerin adlarını listeler; gerçek anahtarları Git'e eklemeyin.

Stok haber, iletişim/iade talepleri ve sipariş/kargo/teslim/iade olayları mağaza sahibine bildirim oluşturur. Yeni üyelikte mağaza sahibine ad, e-posta ve kayıt zamanı; müşteriye karşılama e-postası gönderilir. Üyelik e-posta sahipliğini doğrulamaz; sipariş erişimi e-posta eşleşmesiyle değil, kayıtlı müşteri kimliğiyle denetlenir. Yeni form taleplerinde müşteriye ayrı bir alındı mesajı kaydedilir. E-postalar HTML ve düz metin içerir. Şifre, ödeme kartı veya kimlik numarası e-postaya eklenmez.

Canlı e-posta yapılandırması: `EMAIL_PROVIDER=resend`, yalnızca SisterCraft alan adında gönderim yetkili `RESEND_API_KEY`, `EMAIL_FROM="SisterCraft&Co <info@sistercraftandco.com>"`, `EMAIL_TEST_MODE=false`. Resend ücretsiz planı hesap genelinde günde 100, ayda 3.000 işlem e-postası sağlar; aynı hesaptaki diğer uygulamalar da bu kotayı paylaşır. Müşteri e-postaları kendi adreslerine, mağaza bildirimleri `STORE_NOTIFICATION_EMAIL=ebr.krgl@gmail.com` adresine gider. Gönderici alan adının DKIM ve SPF/MX kayıtları doğrulanmalıdır. Gönderim hizmeti, info adresi için gelen posta kutusu oluşturmaz.

Yönetim → Güvenlik ve açılış bölümünde test e-postası düğmesi, sağlayıcı kabul sayısı, sıra ve kontrol bekleyen bildirimler bulunur. Sağlayıcı kabulü teslim onayı değildir. MailerSend için belirsiz ağ sonucu, duraklatma ve reddedilen gönderimler otomatik tekrarlanmaz; sağlayıcı panelinde kontrol edilir. İzin listesinde olmayan test alıcıları `blocked` durumunda bekler; izin verilince yeniden gönderilebilir. Resend canlı gönderimde kullanılır; aynı idempotency anahtarıyla 23 saat içinde güvenli tekrar yapar. Sağlayıcı ve içerik ilk denemede sabitlenir. API anahtarları sunucuda tutulur. Eski MailerSend desteği korunur. Sağlayıcı değiştiğinde önceden denenmiş mesajlar farklı sağlayıcıyla otomatik tekrar gönderilmez.

Sipariş kabulü; canlı ödeme yapılandırması, kargo ayarları ve mağaza sahibinin açılış seçimiyle yönetilir. Ticari bilgi eksikleri, e-posta durumu ve gerçek ödeme/iade testi yönetimde ayrı kontrol maddeleridir. `/kayit`, `/giris` ve `/hesabim` müşteri hesabı akışlarını sağlar.
