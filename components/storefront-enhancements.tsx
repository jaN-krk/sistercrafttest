...
import {useShop,ProductCard} from './shop';
...
export function StoreHeader(){const{cart,setCartOpen,products,config}=useShop(),{locale,t,format}=useLanguage();const[menu,setMenu]=useState(false),[search,setSearch]=useState(false),[query,setQuery]=useState(''),[scrolled,setScrolled]=useState(false);const path=usePathname(),router=useRouter();useEffect(()=>{setMenu(false);setSearch(false)},[path]);useEffect(()=>{const handler=()=>setScrolled(window.scrollY>15);handler();window.addEventListener('scroll',handler,{passive:true});return()=>window.removeEventListener('scroll',handler)},[]);if(path.startsWith('/yonetim'))return null;const found=products.filter(p=>(p.name+' '+p.subtitle).toLocaleLowerCase(locale).includes(query.toLocaleLowerCase(locale))).slice(0,5);return <Localized><div className="announcement">{locale==='tr'?config.merchandising.announcementTr:config.merchandising.announcementEn}</div><header className={'site-header upgraded-header '+(scrolled?'is-scrolled':'')}><button className="icon-button mobile-menu" aria-label="Menüyü aç" onClick={()=>setMenu(true)}><Menu size={21}/></button><Link href="/" className="wordmark">sistercraft<span>&co.</span></Link><div className="desktop-navigation"><NavigationMenu><NavigationMenuList><NavigationMenuItem><NavigationMenuTrigger>{t('Koleksiyon','Collection')}</NavigationMenuTrigger><NavigationMenuContent><Localized><div className="mega-menu"><div className="mega-links"><span className="eyebrow">{t('SENİN KÜÇÜK RİTÜELİN','YOUR EVERYDAY RITUAL')}</span>{categories.map(c=><NavigationMenuLink key={c.id} render={<Link href={'/koleksiyon'+(c.id==='tumu'?'':'?kategori='+c.id)}/>}>{t(c.name)}<ArrowUpRight size={17}/></NavigationMenuLink>)}</div><Link href="/urun/992963569" className="mega-feature"><img src="/products/992963569_1.jpg" alt="Ritüel Kutusu"/><span>{t('Bir kutu dolusu ritüel','A box of little rituals')}<ArrowUpRight size={17}/></span></Link></div></Localized></NavigationMenuContent></NavigationMenuItem><NavigationMenuItem><NavigationMenuLink render={<Link href="/hikayemiz"/>}>{t('Hikâyemiz','Our story')}</NavigationMenuLink></NavigationMenuItem><NavigationMenuItem><NavigationMenuLink render={<Link href="/gunluk"/>}>{t('Ritüel günlüğü','The ritual journal')}</NavigationMenuLink></NavigationMenuItem></NavigationMenuList></NavigationMenu></div><div className="header-tools"><LanguageSwitch/><button className="icon-button" onClick={()=>setSearch(true)} aria-label="Ürün ara"><Search size={20}/></button><Link className="icon-button account-button" href="/siparisler" aria-label={t('Siparişlerim','My orders')}><UserRound size={20}/></Link><button className="icon-button bag-button" onClick={()=>setCartOpen(true)} aria-label={t('Sepetim','My bag')+` (${cart?.count??0})`}><ShoppingBag size={20}/><span>{cart?.count??0}</span></button></div></header><Sheet open={menu} onOpenChange={setMenu}><SheetContent side="left" className="menu-sheet"><SheetTitle className="wordmark">sistercraft&co.</SheetTitle><SheetDescription>{t('Gününe eşlik edecek küçük ritüeller.','Little rituals for your everyday.')}</SheetDescription><LanguageSwitch/><nav>{categories.map(c=><Link key={c.id} href={'/koleksiyon'+(c.id==='tumu'?'':'?kategori='+c.id)} onClick={()=>setMenu(false)}>{t(c.name)}<ArrowUpRight size={18}/></Link>)}{[['hikayemiz','Hikâyemiz','Our story'],['gunluk','Ritüel günlüğü','Journal'],['siparisler','Siparişlerim','My orders'],['iletisim','İletişim','Contact']].map(([url,tr,en])=><Link key={url} href={'/'+url}>{t(tr,en)}</Link>)}</nav></SheetContent></Sheet><Dialog open={search} onOpenChange={setSearch}><DialogContent className="search-dialog enhanced-search"><DialogTitle>{t('Küçük bir keşfe çık.','Find your next little ritual.')}</DialogTitle><DialogDescription>{t('Koku, ürün veya koleksiyon ara.','Search by scent, product or collection.')}</DialogDescription><form className="search-form" onSubmit={e=>{e.preventDefault();setSearch(false);router.push('/koleksiyon?ara='+encodeURIComponent(query))}}><Input autoFocus value={query} maxLength={100} onChange={e=>setQuery(e.target.value)} aria-label={t('Ürün ara','Search products')} placeholder={t('Vanilya, adaçayı, mum…','Vanilla, sage, candles…')}/><button className="button" aria-label={t('Ara','Search')}><ArrowRight size={19}/></button></form><div className="instant-results">{found.map(p=><Link key={p.id} href={'/urun/'+p.id} onClick={()=>setSearch(false)}><img src={p.images[0]} alt=""/><span>{p.name}<small>{format(p.priceCents)}</small></span><ArrowUpRight size={17}/></Link>)}{!found.length&&<p>{t('Eşleşen ürün bulunamadı.','No matching products.')}</p>}</div></DialogContent></Dialog></Localized>}
